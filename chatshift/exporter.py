...
class ChatExporter:
    ...
    async def export_chat(self, dialog):
        ...
        console.print(f"[bold green]Exporting chat: [/bold green][bold cyan]{chat_title}[/bold cyan]")
        ...
        with Progress(
            ...
            task = progress.add_task(f"[cyan]Downloading messages...", total=None)
            
            # Get messages
            messages = []
            async for message in self.client.iter_messages(entity, limit=limit):
                messages.append(message)
                progress.update(task, advance=1, total=limit if limit > 0 else None)
        ...
        console.print(f"[bold green]Export complete! [/bold green]Saved to [bold cyan]{output_file}[/bold cyan]")
        console.print(f"[bold]Total messages exported: [/bold][bold cyan]{len(messages)}[/bold cyan]")
